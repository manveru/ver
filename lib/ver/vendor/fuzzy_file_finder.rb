...
class FuzzyFileFinder
  ...
  def inspect #:nodoc:
    format('#<%s:0x%x roots=%s, files=%d>', self.class.name, object_id, roots.map { |r| r.name.inspect }.join(', '), files.length)
  end
  ...
  def build_match_result(match, inside_segments)
    ...
    inside_runs = runs.select(&:inside)
  ...
  def match_file(file, file_regex, path_match)
  ...
  def determine_shared_prefix
    ...
    segments = split_roots.map(&:length).max
    master = split_roots.pop

    segments.times do |segment|
      unless split_roots.all? { |root| root[segment] == master[segment] }
