  ...
  [{begin: /^(?=\s*[:.*#a-zA-Z])/,
    ...
    patterns: 
     [{include: "#comment-block"},
      ...
      {captures: {1 => {name: "punctuation.definition.entity.css"}},
       match: 
        /(:+)\b(after|before|first-child|first-letter|first-line|selection)\b/,
      ...
      {begin: /(url)\s*(\()\s*/,
       ...
       patterns: 
        [{match: /[^'") \t]+/, name: "variable.parameter.url.css"},
         ...
         {include: "#string-double"}]}]},
   {begin: 
     /^\s*((@)media)\s+(((all|aural|braille|embossed|handheld|print|projection|screen|tty|tv)\s*,?\s*)+)\s*{/,
    captures: 
      ...
      {begin: /(:)\s*/,
       ...
       patterns: 
        [{match: 
         ...
         {match: /(-|\+)?\s*[0-9]+(\.[0-9]+)?/, name: "constant.numeric.css"},
