...
\s*(\() # the opening parenthesis for arguments/,
    beginCaptures: 
     {1 => {name: "storage.type.octave"},
      2 => {name: "variable.parameter.output.function.octave"},
      3 => {name: "variable.parameter.output.function.octave"},
      4 => {name: "entity.name.function.octave"}},
    contentName: "variable.parameter.input.function.octave",
    end: "\\)",
    endCaptures: {0 => {name: "punctuation.definition.parameters.octave"}},
...
((?>[a-zA-Z_]\w*(?>[?!]|=(?!>))? )) # the function name/,
 ...
 repository: 
  {all_octave_comments: 
    {patterns: 
      [{captures: {1 => {name: "punctuation.definition.comment.octave"}},
        match: /(%%).*$\n?/,
   ...
   brackets: 
    {begin: /\[/,
     beginCaptures: {0 => {name: "meta.brackets.octave"}},
     contentName: "meta.brackets.octave",
     end: "\\]",
     endCaptures: {0 => {name: "meta.brackets.octave"}},
   ...
   curlybrackets: 
    {begin: /\{/,
     beginCaptures: {0 => {name: "meta.brackets.curly.octave"}},
     contentName: "meta.brackets.curly.octave",
     end: "\\}",
     endCaptures: {0 => {name: "meta.brackets.curly.octave"}},
   ...
   escaped_quote: 
    {patterns: [{match: /''/, name: "constant.character.escape.octave"}]},
   not_equal_invalid: 
   ...
   octave_keyword_control: 
    {comment: "Control keywords",
     match: 
      /(?<!\.)\b(all_va_args|break|case|catch|continue|else|end|for|elseif|end_unwind_protect|global|gplot|gsplot|if|otherwise|persistent|replot|return|static|start|startat|stop|switch|try|until|unwind_protect|unwind_protect_cleanup|varargin|varargout|wait|while)\b/,
   ...
   parens: 
    {begin: /\(/,
     beginCaptures: {0 => {name: "meta.parens.octave"}},
     contentName: "meta.parens.octave",
     end: "\\)",
     endCaptures: {0 => {name: "meta.parens.octave"}},
     patterns: [{include: "#allofem"}, {include: "#end_in_parens"}]},
   special_characters: 
    {comment: "Operator symbols",
     match: 
   ...
   string: 
    {begin: /((?<=(\[|\(|\{|=|\s|,|;))|^)'/,
     beginCaptures: 
      {0 => {name: "punctuation.definition.string.begin.octave"}},
     end: "'(?=(\\]|\\)|\\}|=|~|<|>|&|\\||-|\\+|\\*|\\.|\\^|\\||\\s|;|,))",
     endCaptures: {0 => {name: "punctuation.definition.string.end.octave"}},
   ...
   unescaped_quote: 
    {patterns: 
      [{match: /'(?=.)/, name: "invalid.illegal.unescaped-quote.octave"}]},
   variable: 
