...
{fileTypes: ["t2t"],
 ...
 patterns: 
  [{begin: /^%%%\s*$/, end: "^%%%\\s*$\\n?", name: "comment.block.txt2tags"},
   ...
   {match: /__([^\s](|.*?[^\s])_*)__/, name: "markup.underline.txt2tags"},
   {match: /\*\*([^\s](|.*?[^\s])\**)\*\*/, name: "markup.bold.txt2tags"},
   {match: /^\s*={1}[^=](|.*[^=])={1}(\[[\w-]*\])?\s*$\n?/,
   ...
   {match: /^\s*\+{5}[^+](|.*[^+])\+{5}(\[[\w-]*\])?\s*$\n?/,
    name: "markup.heading.5.txt2tags"},
   {match: /\/\/([^\s](|.*?[^\s])\/*)\/\//, name: "markup.italic.txt2tags"},
   {match: /--([^\s](|.*?[^\s])-*)--/,
   ...
   {match: /^ *: (?=.)/, name: "markup.list.unnumbered.txt2tags"},
   {match: /^( *)([-+:])\s*$/, name: "markup.list.txt2tags"},
   {begin: /^```\s*$/,
    end: "^```\\s*$\\n?",
