...
{fileTypes: ["lgt"],
 ...
 patterns: 
  [{begin: /\/\*/,
   ...
   {captures: {1 => {name: "punctuation.definition.storage.modifier.logtalk"}},
    match: 
     /(:-)\s(alias|info|d(ynamic|iscontiguous)|m(eta_predicate|ode|ultifile)|p(ublic|r(otected|ivate))|op|use(s|_module)|synchronized)(?=[(])/,
    name: "storage.modifier.others.logtalk"},
   ...
   {match: /(<<|>>|\/\\|\\\/|\\)/, name: "keyword.operator.bitwise.logtalk"},
   {match: /\b(mod|rem)\b(?![-!(^~])/,
   ...
   {match: 
     /\b(a(bs|tan)|c(eiling|os)|exp|flo(at(_(integer|fractional)_part)?|or)|log|mod|r(em|ound)|s(i(n|gn)|qrt)|truncate)(?=[(])/,
   ...
   {match: /\b(var|atom(ic)?|integer|float|compound|n(onvar|umber))(?=[(])/,
   ...
   {match: 
     /\b((object|protocol|category)_property|complements_object|extends_(object|protocol|category)|imp(orts_category|lements_protocol)|(instantiat|specializ)es_class)(?=[(])/,
   ...
   {match: /\b(0b[0-1]+|0o[0-7]+|0x\h+)\b/, name: "constant.numeric.logtalk"},
