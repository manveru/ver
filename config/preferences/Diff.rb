# Encoding: UTF-8

[{name: "Style: Changed",
  scope: "markup.changed",
  settings: {background: "#F5C411", foreground: "#FFFFFF"},
  uuid: "EB0FD215-C2EE-4576-86FD-ABA3EDCB5A7B"},
 {name: "Style: Deleted",
  scope: "markup.deleted",
  settings: {background: "#FF3D3D", foreground: "#FFFFFF"},
  uuid: "FFF345C5-D3B4-4975-A610-69CC645FEE7C"},
 {name: "Style: Index",
  scope: "meta.diff.index",
  settings: 
   {background: "#3467D1", fontStyle: "italic", foreground: "#FFFFFF"},
  uuid: "F9C2B477-B6F7-4951-953B-32BFEF121F7C"},
 {name: "Style: Header",
  scope: "meta.diff.header",
  settings: {background: "#5685E3", foreground: "#FFFFFF"},
  uuid: "ED5636AE-09EF-46C4-A4F8-B88F701763E2"},
 {name: "Style: Inserted",
  scope: "markup.inserted",
  settings: {background: "#73FF65", foreground: "#000000"},
  uuid: "7D036841-43BD-49CA-892F-0A6837EF8FB7"},
 {name: "Style: Range",
  scope: "meta.diff.range",
  settings: 
   {background: "#3467D1", fontStyle: "italic", foreground: "#FFFFFF"},
  uuid: "71A420A8-0892-4622-BDA4-CC00D98C59E6"},
 {name: "Symbol List: Indent Ranges",
  scope: "source.diff meta.diff.range",
  settings: 
   {comment: 
     "Insert (figure space) padding\n\t\tso that “+nnn,n” becomes “+ nnn, n”",
    symbolTransformation: 
     "\n\t\ts/[-+](\\d)(\\d)?(\\d)?(\\d)?,/(?4:: (?3:: (?2:: )))$0/g;\n\t\ts/,(\\d)\\b/, $1/g;\n\t\ts/^/ /;\n\t"},
  uuid: "A0B3B6D2-59B1-407F-B310-7FA67222B37A"}]
