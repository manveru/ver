# Encoding: UTF-8

{comment: "Ober (modified Sunburst)",
 name: "newLISP",
 settings: 
  [{settings: 
     {background: "#FFFFFF",
      caret: "#A7A7A7",
      foreground: "#000000",
      invisibles: "#CAE2FB3D",
      lineHighlight: "#FFFFFF0D",
      selection: "#C7D8E2"}},
   {name: "Comment",
    scope: "comment",
    settings: {fontStyle: "", foreground: "#555555"}},
   {name: "Constant", scope: "constant", settings: {foreground: "#685500"}},
   {name: "Entity",
    scope: "entity",
    settings: {fontStyle: "bold", foreground: "#090909"}},
   {name: "Keyword",
    scope: "keyword",
    settings: {fontStyle: "bold", foreground: "#1F27B7"}},
   {name: "Storage",
    scope: "storage",
    settings: {fontStyle: "bold", foreground: "#1F27B7"}},
   {name: "String",
    scope: "string",
    settings: {fontStyle: "", foreground: "#006801"}},
   {name: "Support",
    scope: "support",
    settings: {fontStyle: "", foreground: "#1F27B7"}},
   {name: "Variable", scope: "variable", settings: {foreground: "#1F27B7"}},
   {name: "Invalid – Deprecated",
    scope: "invalid.deprecated",
    settings: 
     {background: "#FFFFB9",
      fontStyle: "italic underline",
      foreground: "#808000"}},
   {name: "Invalid – Illegal",
    scope: "invalid.illegal",
    settings: {background: "#800000", foreground: "#FFFFFF"}},
   {name: "-----------------------------------", settings: {}},
   {name: "♦ Embedded Source (Bright)",
    scope: "text source",
    settings: {background: "#B1B3BA08"}},
   {name: "♦ Entity inherited-class",
    scope: "entity.other.inherited-class",
    settings: {fontStyle: "", foreground: "#9B5C2E"}},
   {name: "♦ String embedded-source",
    scope: "string.quoted source",
    settings: {fontStyle: "", foreground: "#A38345"}},
   {name: "♦ String constant",
    scope: "string constant",
    settings: {fontStyle: "", foreground: "#A38345"}},
   {name: "♦ String.regexp",
    scope: "string.regexp",
    settings: {foreground: "#A38345"}},
   {name: "♦ String.regexp.«special»",
    scope: 
     "string.regexp constant.character.escape, string.regexp source.ruby.embedded, string.regexp string.regexp.arbitrary-repitition",
    settings: {foreground: "#CF7D34"}},
   {name: "♦ String variable",
    scope: "string variable",
    settings: {foreground: "#8A9A95"}},
   {name: "♦ Support.function",
    scope: "support.function",
    settings: {fontStyle: "", foreground: "#1F27B7"}},
   {name: "♦ Support.constant",
    scope: "support.constant",
    settings: {fontStyle: "", foreground: "#685500"}},
   {name: "c C/C++ Preprocessor Line",
    scope: "meta.preprocessor.c",
    settings: {foreground: "#8996A8"}},
   {name: "c C/C++ Preprocessor Directive",
    scope: "meta.preprocessor.c keyword",
    settings: {foreground: "#AFC4DB"}},
   {name: "j Entity Name Type",
    scope: "entity.name.type",
    settings: {fontStyle: "bold"}},
   {name: "j Cast",
    scope: "meta.cast",
    settings: {fontStyle: "italic", foreground: "#676767"}},
   {name: "✘ Doctype/XML Processing",
    scope: 
     "meta.sgml.html meta.doctype, meta.sgml.html meta.doctype entity, meta.sgml.html meta.doctype string, meta.xml-processing, meta.xml-processing entity, meta.xml-processing string",
    settings: {foreground: "#494949"}},
   {name: "✘ Meta.tag.«all»",
    scope: "meta.tag, meta.tag entity",
    settings: {fontStyle: "", foreground: "#006801"}},
   {name: "✘ Meta.tag.inline",
    scope: 
     "source entity.name.tag, source entity.other.attribute-name, meta.tag.inline, meta.tag.inline entity",
    settings: {fontStyle: "", foreground: "#685500"}},
   {name: "✘ Namespaces",
    scope: "entity.name.tag.namespace, entity.other.attribute-name.namespace",
    settings: {fontStyle: "", foreground: "#B71A18"}},
   {name: "§ css tag-name",
    scope: "meta.selector.css entity.name.tag",
    settings: {fontStyle: "", foreground: "#B71A18"}},
   {name: "§ css:pseudo-class",
    scope: "meta.selector.css entity.other.attribute-name.tag.pseudo-class",
    settings: {foreground: "#8F9D6A"}},
   {name: "§ css#id",
    scope: "meta.selector.css entity.other.attribute-name.id",
    settings: {foreground: "#000000"}},
   {name: "§ css.class",
    scope: "meta.selector.css entity.other.attribute-name.class",
    settings: {fontStyle: "", foreground: "#000000"}},
   {name: "§ css property-name:",
    scope: "support.type.property-name.css",
    settings: {fontStyle: "bold", foreground: "#1F27B7"}},
   {name: "§ css property-value;",
    scope: 
     "meta.property-group support.constant.property-value.css, meta.property-value support.constant.property-value.css",
    settings: {foreground: "#000000"}},
   {name: "§ css @at-rule",
    scope: "meta.preprocessor.at-rule keyword.control.at-rule",
    settings: {foreground: "#8693A5"}},
   {name: "§ css additional-constants",
    scope: 
     "meta.property-value support.constant.named-color.css, meta.property-value constant",
    settings: {foreground: "#685500"}},
   {name: "§ css constructor.argument",
    scope: "meta.constructor.argument.css",
    settings: {foreground: "#8F9D6A"}},
   {name: "⎇ diff.header",
    scope: "meta.diff, meta.diff.header",
    settings: 
     {background: "#0E2231", fontStyle: "italic", foreground: "#F8F8F8"}},
   {name: "⎇ diff.deleted",
    scope: "markup.deleted",
    settings: {background: "#420E09", foreground: "#F8F8F8"}},
   {name: "⎇ diff.changed",
    scope: "markup.changed",
    settings: {background: "#4A410D", foreground: "#F8F8F8"}},
   {name: "⎇ diff.inserted",
    scope: "markup.inserted",
    settings: {background: "#253B22", foreground: "#F8F8F8"}},
   {name: "--------------------------------", settings: {}},
   {name: "Markup: Italic",
    scope: "markup.italic",
    settings: {fontStyle: "italic", foreground: "#CB0202"}},
   {name: "Markup: Bold",
    scope: "markup.bold",
    settings: {fontStyle: "bold", foreground: "#CB0202"}},
   {name: "Markup: Underline",
    scope: "markup.underline",
    settings: {fontStyle: "underline", foreground: "#CB0202"}},
   {name: "Markup: Quote",
    scope: "markup.quote",
    settings: 
     {background: "#F8F7E4", fontStyle: "italic", foreground: "#408000"}},
   {name: "Markup: Heading",
    scope: "markup.heading, markup.heading entity",
    settings: {background: "#632D04", fontStyle: "", foreground: "#FEDCC5"}},
   {name: "Markup: List",
    scope: "markup.list",
    settings: {foreground: "#908978"}},
   {name: "Markup: Raw",
    scope: "markup.raw",
    settings: {background: "#B1B3BA08", fontStyle: "", foreground: "#26368F"}},
   {name: "Markup: Comment",
    scope: "markup comment",
    settings: {fontStyle: "italic", foreground: "#F67B37"}},
   {name: "Markup: Separator",
    scope: "meta.separator",
    settings: {background: "#242424", foreground: "#60A633"}},
   {name: "Log Entry",
    scope: "meta.line.entry.logfile, meta.line.exit.logfile",
    settings: {background: "#EEEEEE29"}},
   {name: "Log Entry Error",
    scope: "meta.line.error.logfile",
    settings: {background: "#751012"}},
   {name: "Namespace",
    scope: "namespace.lsp",
    settings: {background: "#94410A", fontStyle: "bold"}},
   {name: "newLISP: paren",
    scope: "operator.lsp",
    settings: {fontStyle: "", foreground: "#B71A18"}},
   {name: "String Delimiter",
    scope: "punctuation.definition.string.lsp",
    settings: {fontStyle: "", foreground: "#006801"}}],
 uuid: "1EB2144B-4567-4860-B4B1-890DA804195C"}
